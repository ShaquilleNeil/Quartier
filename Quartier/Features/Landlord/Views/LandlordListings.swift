...
struct LandlordListings: View {
    ...
    @FetchRequest(
        ...
        predicate: NSPredicate(format: "status == %@", "draft"),
        animation: .default
    )
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottomTrailing) {
                ...
                NavigationLink(destination: ListingFormView(), isActive: $isAddingListing) {
                    EmptyView()
                }
            }
        }
    ...
    // MARK: - Draft View (CoreData)

    var draftListView: some View {
        VStack(spacing: 12) {
            ForEach(draftListings.filter { !$0.isRented }, id: \.objectID) { item in
                NavigationLink(destination: ListingFormView(existingListing: convertDraftToListing(item))) {
    ...
    // MARK: - Published View (Firebase)

    var publishedListView: some View {
        VStack(spacing: 12) {
            ForEach(firebase.firebaseListings.filter { $0.status.lowercased() == "published" && !$0.isEffectivelyRented }) { item in
                NavigationLink(destination: ListingFormView(existingListing: convertToEditableListing(item))) {
                    remoteCard(item)
                }
                .buttonStyle(.plain)
            }
    ...
    // MARK: - Rented View (Firebase)

    var rentedListView: some View {
        VStack(spacing: 12) {
            ForEach(firebase.firebaseListings.filter { $0.isEffectivelyRented }) { item in
                NavigationLink(destination: ListingFormView(existingListing: convertToEditableListing(item))) {
                    remoteCard(item)
                }
                .buttonStyle(.plain)
    ...
    func draftCard(_ item: LDListing, rentedLocal: Bool) -> some View {
        HStack(spacing: 14) {
            if let images = item.draftImages as? Set<DraftImage>,
               ...
               let uiImage = UIImage(data: data) {
                ...
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .frame(width: 85, height: 85)
                    .overlay(Image(systemName: "photo").font(.system(size: 22, weight: .semibold)).foregroundStyle(.orange))
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Text(rentedLocal ? "Rented (draft)" : "Draft")
                        ...
                        .padding(.vertical, 6)
                        .background(Capsule().fill((rentedLocal ? Color.blue : Color.orange).opacity(0.18)))
                }
                Text(formattedPrice(item.price) + " • \(item.bedrooms) bds • \(item.bathrooms) ba")
                    .font(.system(size: 13))
        ...
        .overlay(RoundedRectangle(cornerRadius: 22).stroke(Color.orange.opacity(0.25), lineWidth: 1))
        .shadow(color: Color.black.opacity(0.08), radius: 10, x: 0, y: 6)
    }

    func remoteCard(_ item: RemoteListing) -> some View {
        HStack(spacing: 14) {
            if let firstURL = item.imageURLs.first, let url = URL(string: firstURL) {
                WebImage(url: url)
                    .onFailure { error in print("Image load failed:", error.localizedDescription) }
                    .resizable()
                    .indicator(.activity)
                ...
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .frame(width: 85, height: 85)
                    .overlay(Image(systemName: "photo").font(.system(size: 22, weight: .semibold)).foregroundStyle(.gray))
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Text(item.status.capitalized)
                        ...
                        .padding(.horizontal, 10)
                        .padding(.vertical, 6)
                        .background(Capsule().fill(statusColor(for: item.status).opacity(0.18)))
                }
                Text(formattedPrice(item.price) + " • \(item.bedrooms) bds • \(item.bathrooms) ba")
                    .font(.system(size: 13))
                    .foregroundStyle(.secondary)
        ...
        .padding(16)
        .background(RoundedRectangle(cornerRadius: 22).fill(.ultraThinMaterial))
        .overlay(RoundedRectangle(cornerRadius: 22).stroke(statusColor(for: item.status).opacity(0.25), lineWidth: 1))
        .shadow(color: Color.black.opacity(0.08), radius: 10, x: 0, y: 6)
    }

    // MARK: - Helpers

    func convertDraftToListing(_ draft: LDListing) -> Listing {
    ...
    func convertDraftImages(_ draft: LDListing) -> [UIImage] {
    ...
    func convertToEditableListing(_ remote: RemoteListing) -> Listing {
        var listing = Listing(
